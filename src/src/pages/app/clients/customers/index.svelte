...
<script>
    ...
    function onItemSelected(event) {
        $goto("/app/clients/customers/:slug", {
    ...
    async function onDelete() {
        if (checked.length > 0) {
            ...
            if (prompt.response == 1) {
                ...
                await window.clients.deleteManyCustomers({ ids });
                refresh = true;
                checked = [];
            } else {
                return;
...
<div class="content" in:fly={{ x: -100 }}>
    <Card>
        <Header slot="header" title="Customers">
            <div class="btn-group" slot="controls">
                <button
                    class="btn"
                    on:click={() => {
                        $goto("/app/clients/customers/add");
                    }}
                >
                    Add
                </button>
        ...
        {#key refresh}
            {#await window.clients.getCustomers() then data}
                <DataTable
                    {columns}
                    rows={data.values}
